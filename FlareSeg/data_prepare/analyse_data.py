...
SAVE_DIR = os.path.join(BASE_DIR, "analyse_data_figures/")
...
def analyse_mask(image_dir, mask_dir, series_ids, suffix=".nii.gz"):
    ...
    for idx, file_name in enumerate(series_ids):
        ...
        if os.path.exists(image_path) and os.path.exists(mask_path):
            ...
            for i, organ_name in enumerate(organ_name_list):
                ...
                mask_properties_dict[organ_name]["intensity"].append(
                    intensity_features[i]
                )

                centroid_shift = [
                    centroid_features[i, 0] - start_centroid_z,
    ...
    plot_properties(
        mask_properties_dict["abdomen_size"], "abdomen_size", ["z", "y", "x"], bins=80
    )
    for organ_name in organ_name_list:
        ...
        plot_properties(
            centroid_features, organ_name, ["z_shift", "y_shift", "x_shift"]
        )
        plot_properties(
            intensity_features,
            organ_name,
        ...
        plot_properties(
            scale_features,
            organ_name,
            ["major_axis_length", "minor_axis_length", "ratio"],
        )
...
def get_properties(npy_image, npy_mask, spacing):
    ...
    for i in range(len(region_props)):
        ...
        volume_ratio.append(
            region_props[i].area
            * 1.0
            / (mask_raw_size[0] * mask_raw_size[1] * mask_raw_size[2])
        )
...
def plot_properties(properties, name, labels, bins="auto"):
    ...
    if len(labels) == 3:
        ...
        plt.savefig("{}.png".format(SAVE_DIR + name + "_" + "_".join(labels)))

        print(
            "{} mean is {}:{}, {}:{}, {}:{}".format(
                name,
                labels[0],
                np.mean(properties[:, 0]),
                labels[1],
                np.mean(properties[:, 1]),
                labels[2],
                np.mean(properties[:, 2]),
            )
        )
        print(
            "{} std is {}:{}, {}:{}, {}:{}".format(
...
if __name__ == "__main__":
    ...
    image_dir = "/mnt/HDD2/safal/EfficientSegmentation/FlareSeg/dataset/train_images"
    mask_dir = "/mnt/HDD2/safal/EfficientSegmentation/FlareSeg/dataset/train_mask"
    file_path = "/mnt/HDD2/safal/EfficientSegmentation/FlareSeg/dataset/file_list/train_series_uids.txt"
