...
def debug_plot(image_slice:sitk.Image, template_slice:sitk.Image, outTx:sitk.sitkTranslation,transform_list,correlations):
  ...
  ax3.sharey(ax)
  ax3.plot(correlations,np.array([y for x,y in transform_list]))
...
if __name__ == '__main__':
  assert 0 == get_saggital_dim('LPS'), 'get_axial_dim() failed'
  assert 0 == get_saggital_dim('RAI'), 'get_axial_dim() failed'  
  assert 1 == get_saggital_dim('PRI'), 'get_axial_dim() failed'  