...
class SegDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == "test":
            ...
            if not self.cfg.COARSE_MODEL.IS_PREPROCESS:
                zoom_image, zoom_factor = ScipyResample.resample_to_size(
                    raw_image, self.cfg.COARSE_MODEL.INPUT_SIZE
                )
                if self.cfg.DATA_LOADER.IS_NORMALIZATION_HU:
                    norm_image = clip_and_normalize_mean_std(
                        zoom_image, self.window_level[0], self.window_level[1]
                    )
                else:
                    norm_image = normalize_min_max_and_clip(
                        zoom_image, self.window_level[0], self.window_level[1]
                    )
            else:
            ...
            if self.cfg.DATA_AUGMENT.IS_RANDOM_ROTATE:
                npy_image, npy_mask = data_augmentor.random_rotate(
                    npy_image,
                    npy_mask,
            ...
            if self.cfg.DATA_LOADER.IS_COARSE:
                if self.cfg.DATA_AUGMENT.IS_RANDOM_SHIFT and np.random.choice([0, 1]):
                    npy_image, npy_mask = data_augmentor.random_shift(
                        npy_image, npy_mask, self.cfg.DATA_AUGMENT.SHIFT_MAX_RATIO
                    )
                out_size = self.cfg.COARSE_MODEL.INPUT_SIZE
            else:
                if self.cfg.DATA_AUGMENT.IS_RANDOM_CROP_TO_LABELS and np.random.choice(
                    [0, 1]
                ):
                    margin = [
                        int(self.cfg.DATA_AUGMENT.MAX_EXTEND_SIZE / image_spacing[0]),
                        int(self.cfg.DATA_AUGMENT.MAX_EXTEND_SIZE / image_spacing[1]),
                        int(self.cfg.DATA_AUGMENT.MAX_EXTEND_SIZE / image_spacing[2]),
                    ]
                    npy_image, npy_mask = data_augmentor.random_crop_to_extend_labels(
                        npy_image, npy_mask, margin
                    )
                else:
                    margin = [
                        int(self.cfg.DATA_LOADER.EXTEND_SIZE / image_spacing[0]),
                        int(self.cfg.DATA_LOADER.EXTEND_SIZE / image_spacing[1]),
                        int(self.cfg.DATA_LOADER.EXTEND_SIZE / image_spacing[2]),
                    ]
                    npy_image, npy_mask = crop_image_according_to_mask(
                        npy_image, npy_mask, margin
                    )
            ...
            npy_mask, _ = ScipyResample.resample_mask_to_size(
                npy_mask, out_size, num_label=np.max(np.array(self.label))
            )
            npy_mask = Binarized(npy_mask)

            if self.cfg.DATA_AUGMENT.IS_ELASTIC_TRANSFORM and np.random.choice([0, 1]):
                npy_image, npy_mask = data_augmentor.elastic_transform_3d(
                    npy_image, npy_mask
                )

            if self.cfg.DATA_AUGMENT.IS_CHANGE_ROI_HU:
                npy_image = data_augmentor.augment_brightness_additive(
                    npy_image,
                    npy_mask,
        ...
        if self.cfg.DATA_LOADER.IS_NORMALIZATION_HU:
            npy_image = clip_and_normalize_mean_std(
                npy_image, self.window_level[0], self.window_level[1]
            )
        else:
            npy_image = normalize_min_max_and_clip(
                npy_image, self.window_level[0], self.window_level[1]
            )

        if (
            self.phase == "train"
            and self.is_data_augment
            and self.cfg.DATA_AUGMENT.IS_ADD_GAUSSIAN_NOISE
            and np.random.choice([0, 1])
        ):
            data_augmentor = DataAugmentor()
            npy_image = data_augmentor.augment_gaussian_noise(
                npy_image, noise_variance=(0, 0.1)
            )

        image_czyx = npy_image[
            np.newaxis,
        ]
        ...
        if self.cfg.FINE_MODEL.AUXILIARY_TASK:
            is_single_channel = (
                True if self.cfg.FINE_MODEL.AUXILIARY_CLASS == 1 else False
            )
            heat_maps = generate_heatmap(
                mask_czyx,
                self.cfg.DATA_LOADER.LABEL_NUM,
                sigma=7,
                stride=4,
                is_single_channel=is_single_channel,
            )
            return (
                torch.from_numpy(image_czyx).float(),
    ...
    def _read_db(self):
        ...
        if (
            self.phase == "train"
            and bad_case_path is not None
            and os.path.exists(bad_case_path)
        ):
