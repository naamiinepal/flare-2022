...
from Common.mask_process import (
    extract_bbox,
...
class BaseSegmentation3D(object):
    def __init__(self, cfg, phase="train"):
        ...
        if self.cfg.DATA_LOADER.IS_COARSE and phase == "train":
            self.train_save_dir = os.path.join(
                ...
                str(self.cfg.ENVIRONMENT.EXPERIMENT_NAME)
                + "_"
                + str(self.cfg.COARSE_MODEL.META_ARCHITECTURE)
                + "_"
                + str(self.cfg.COARSE_MODEL.ENCODER_CONV_BLOCK)
                + "_"
                + str(self.cfg.COARSE_MODEL.DECODER_CONV_BLOCK)
                + "_"
                + str(self.cfg.COARSE_MODEL.CONTEXT_BLOCK)
                + "_fold-"
                + str(self.cfg.DATA_LOADER.TRAIN_VAL_FOLD)
                + "_coarse_size-"
                + str(self.cfg.COARSE_MODEL.INPUT_SIZE[0])
                + "_channel-"
                + str(self.cfg.COARSE_MODEL.NUM_CHANNELS[0])
                + "_depth-"
                + str(self.cfg.COARSE_MODEL.NUM_DEPTH)
                + "_loss-"
                + self.cfg.TRAINING.LOSS
                + "_metric-"
                + self.cfg.TRAINING.METRIC
                + "_time-"
            ...
            self.train_save_dir = os.path.join(
                ...
                str(self.cfg.ENVIRONMENT.EXPERIMENT_NAME)
                + "_"
                + str(self.cfg.FINE_MODEL.META_ARCHITECTURE)
                + "_"
                + str(self.cfg.FINE_MODEL.ENCODER_CONV_BLOCK)
                + "_"
                + str(self.cfg.FINE_MODEL.DECODER_CONV_BLOCK)
                + "_"
                + str(self.cfg.FINE_MODEL.CONTEXT_BLOCK)
                + "_fold-"
                + str(self.cfg.DATA_LOADER.TRAIN_VAL_FOLD)
                + "_fine_size-"
                + str(self.cfg.FINE_MODEL.INPUT_SIZE[0])
                + "_channel-"
                + str(self.cfg.FINE_MODEL.NUM_CHANNELS[0])
                + "_depth-"
                + str(self.cfg.FINE_MODEL.NUM_DEPTH)
                + "_loss-"
                + self.cfg.TRAINING.LOSS
                + "_metric-"
                + self.cfg.TRAINING.METRIC
        ...
        if self.is_distributed_train:
            ...
            self.is_print_out = (
                True if local_rank == 0 else False
            )  # Only GPU 0 print information.
        ...
        # step 3 >>> init logger
        self.log_dir = (
            os.path.join(self.train_save_dir, "logs")
            if phase == "train"
            else os.path.join(self.test_save_dir, "logs")
        )
    ...
    def _load_weights(self):
        if self.phase == "test":
            ...
            if self.coarse_model_weight_dir is not None and os.path.exists(
                self.coarse_model_weight_dir
            ):
            ...
            if self.fine_model_weight_dir is not None and os.path.exists(
                self.fine_model_weight_dir
            ):
            ...
            self.weight_dir = (
                self.cfg.COARSE_MODEL.WEIGHT_DIR
                if self.cfg.DATA_LOADER.IS_COARSE
                else self.cfg.FINE_MODEL.WEIGHT_DIR
            )
    ...
    def _get_lr_scheduler(self):
        if self.cfg.TRAINING.SCHEDULER.LR_SCHEDULE == "cosineLR":
            self.scheduler = optim.lr_scheduler.CosineAnnealingLR(
                self.optimizer, T_max=30, eta_min=3e-5
            )
        elif self.cfg.TRAINING.SCHEDULER.LR_SCHEDULE == "stepLR":
            self.scheduler = optim.lr_scheduler.StepLR(
                self.optimizer, step_size=60, gamma=0.5
            )
    ...
    def get_gpu_memory_usage(
        time_interval, is_ongoing_monitor=True, gpu_index=0, out_path=None
    ):
        gpu_memory_max = 0
        ...
        while is_ongoing_monitor:
            ...
            gpu_memory = (
                dictm["gpu"][int(gpu_index)]["fb_memory_usage"]["total"]
            ...
            memory_metric_min = min(
                (gpu_memory_total - gpu_memory_max) * 1.0 / gpu_memory_total,
                memory_metric_min,
            )
            if is_ongoing_monitor:
                ...
                if interval_nums % 20 == 0:
                    if out_path is not None:
                        write_txt(
                            out_path, [str(gpu_memory_max), str(memory_metric_min)]
                        )
    ...
    def do_train(self):
        ...
        if self.is_print_out:
            ...
            self.logger.info(
                "train samples per epoch: {}".format(len(self.train_loader))
            )
        ...
        for epoch in range(
            self.start_epoch, self.cfg.TRAINING.SCHEDULER.TOTAL_EPOCHS + 1
        ):
            if self.is_print_out:
            ...
            if self.is_print_out:
                if val_dice > best_dice:
                    best_dice = val_dice
                    self._save_weights(
                        epoch, self.model.state_dict(), self.optimizer.state_dict()
                    )
    ...
    def _train(self, epoch):
        ...
        for param_group in self.optimizer.param_groups:
            param_group["lr"] = self._get_lr(
                epoch, self.cfg.TRAINING.SCHEDULER.TOTAL_EPOCHS, self.lr
            )

        train_dice = [0.0] * len(self.cfg.DATA_LOADER.LABEL_INDEX)
        train_total = 0
        ...
        for index, (images, masks) in enumerate(self.train_loader):
            ...
            if self.cfg.FINE_MODEL.DEEP_SUPERVISION:
                ...
                seg_loss = 0
                loss_func = SegLoss(
                    loss_func=self.cfg.TRAINING.LOSS,
            ...
            if self.is_print_out:
                if index > 0 and index % self.cfg.TRAINING.SAVER.SAVER_FREQUENCY == 0:
                    self.logger.info(
                        "Epoch: {}/{} [{}/{} ({:.0f}%)]".format(
                            epoch,
                            self.cfg.TRAINING.SCHEDULER.TOTAL_EPOCHS,
                            index * len(images),
                            len(self.train_loader),
                            100.0 * index / len(self.train_loader),
                        )
                    )
                    ...
                    for i, dice_label in enumerate(train_dice):
                        ...
                        self.logger.info(
                            "{} Dice:{:.6f}".format(
                                self.cfg.DATA_LOADER.LABEL_NAME[i], dice_ind
                            )
                        )
        ...
        if self.is_print_out:
            ...
            for i, dice_label in enumerate(train_dice):
                ...
                self.train_writer.add_scalars(
                    "Train/Dice", {self.cfg.DATA_LOADER.LABEL_NAME[i]: dice_ind}, epoch
                )

    def _validate(self, epoch):
        self.model.eval()

        val_dice = [0.0] * len(self.cfg.DATA_LOADER.LABEL_INDEX)
        val_total = 0
        val_loss = 0
        if self.cfg.TRAINING.METRIC == "diceAndHausdorff":
            is_hausdorff = True
            val_hausdorff = [0.0] * len(self.cfg.DATA_LOADER.LABEL_INDEX)
        else:
            is_hausdorff = False
        ...
        for images, masks in self.val_loader:
            ...
            loss_func = SegLoss(
                loss_func="dice", activation=self.cfg.TRAINING.ACTIVATION
            )
            ...
            if is_hausdorff:
                ...
                for i in range(batch_num):
                    area_dice, surface_dice = compute_flare_metric(
                        masks[i], output_seg[i], [1.0, 1.0, 1.0]
                    )
                ...
                hausdorff_output = (
                    torch.from_numpy(hausdorff_output).float().to(self.device)
                )
            else:
                dice_metric_func = DiceMetric()
                dice_output = dice_metric_func(
        ...
        if self.is_print_out:
            ...
            for idx, _ in enumerate(val_dice):
                ...
                self.logger.info(
                    "{} Dice:{:.6f}".format(
                        self.cfg.DATA_LOADER.LABEL_NAME[idx], val_dice[idx]
                    )
                )
                ...
                if is_hausdorff:
                    ...
                    self.logger.info(
                        "{} Hausdorff:{:.6f}".format(
                            self.cfg.DATA_LOADER.LABEL_NAME[idx], val_hausdorff[idx]
                        )
                    )
                    self.val_writer.add_scalars(
                        "Val/Hausdorff",
    ...
    def do_test(self):
        ...
        total_content = [
            "Average_data_loader_time",
            "Average_coarse_infer_time",
            "Average_coarse_postprocess_time",
            "Average_fine_infer_time",
            "Average_time_usage",
            "Max_memory_usage",
            "Time_score",
            "Memory_score",
        ...
        gpu_memory_txt_path = os.path.join(
            self.test_save_dir, "max_gpu_memory_usage.txt"
        )
        p = Process(
            target=self.get_gpu_memory_usage,
            args=(
                self.cfg.ENVIRONMENT.MONITOR_TIME_INTERVAL,
                True,
        ...
        for batch_idx, data_dict in enumerate(test_dataloader):
            ...
            coarse_image = torch.from_numpy(
                coarse_image[np.newaxis, np.newaxis]
            ).float()
            if self.cfg.ENVIRONMENT.CUDA:
                coarse_image = (
                    coarse_image.half() if self.cfg.TESTING.IS_FP16 else coarse_image
                )
            ...
            pred_coarse_mask = torch.where(
                pred_coarse_mask >= 0.5, torch.tensor(1), torch.tensor(0)
            )
            ...
            if self.cfg.TESTING.IS_POST_PROCESS:
                ...
                coarse_spacing = [
                    raw_spacing[i] * coarse_zoom_factor[i] for i in range(3)
                ]
                area_least = (
                    1000 / coarse_spacing[0] / coarse_spacing[1] / coarse_spacing[2]
                )
                out_coarse_mask = extract_topk_largest_candidates(
                    pred_coarse_mask, label_num, area_least
                )
            else:
            ...
            raw_bbox = [
                int(coarse_bbox[0][0] * coarse_zoom_factor[0]),
            ...
            margin = [
                self.cfg.DATA_LOADER.EXTEND_SIZE / raw_spacing[i] for i in range(3)
            ]
            ...
            time_consume["coarse_postprocess"].append(
                t_coarse_postprocess_end - t_coarse_infer_end
            )
            ...
            fine_zoom_factor = crop_image_size / np.array(
                self.cfg.FINE_MODEL.INPUT_SIZE
            )
            if not self.cfg.FINE_MODEL.IS_PREPROCESS:
                crop_image, _ = ScipyResample.resample_to_size(
                    crop_image, self.cfg.FINE_MODEL.INPUT_SIZE, order=1
                )
                crop_image = clip_and_normalize_mean_std(
                    crop_image,
                    self.cfg.DATA_LOADER.WINDOW_LEVEL[0],
            ...
            if self.cfg.ENVIRONMENT.CUDA:
                crop_image = (
                    crop_image.half() if self.cfg.TESTING.IS_FP16 else crop_image
                )
            ...
            if self.cfg.FINE_MODEL.IS_POSTPROCESS:
                predict_fine_mask = torch.where(
                    predict_fine_mask >= 0.5, torch.tensor(1), torch.tensor(0)
                )
            predict_fine_mask = predict_fine_mask.numpy().squeeze(axis=0)
            torch.cuda.synchronize()
            t_fine_infer_end = time.time()
            time_consume["fine_infer"].append(
                t_fine_infer_end - t_coarse_postprocess_end
            )

            time_usage = {
                "data_loader": time_consume["data_loader"][-1],
        ...
        average_coarse_postprocess = np.mean(
            np.array(time_consume["coarse_postprocess"])
        )
        average_fine_infer = np.mean(np.array(time_consume["fine_infer"]))
        self.logger.info(
            "Average data loader time: {}".format(average_data_loader_time)
        )
        self.logger.info("Average coarse infer time: {}".format(average_coarse_infer))
        self.logger.info(
            "Average coarse postprocess time: {}".format(average_coarse_postprocess)
        )
        self.logger.info("Average fine infer time: {}".format(average_fine_infer))

        out_content = [
            average_data_loader_time,
            average_coarse_infer,
            average_coarse_postprocess,
            average_fine_infer,
            average_time_usage,
            max_memory_usage,
            time_score,
    ...
    def _post_process(
        ...
        if not self.cfg.FINE_MODEL.IS_POSTPROCESS:
            ...
            for i in range(len(predict)):
                mask, _ = ScipyResample.resample_to_size(
                    predict[i],
                    crop_image_size,
        ...
        if self.cfg.TESTING.IS_POST_PROCESS:
            ...
            predict = extract_topk_largest_candidates(
                predict, self.cfg.DATA_LOADER.LABEL_NUM, area_least
            )
        else:
        ...
        out_mask[
            crop_fine_bbox[0] : crop_fine_bbox[1],
            crop_fine_bbox[2] : crop_fine_bbox[3],
            crop_fine_bbox[4] : crop_fine_bbox[5],
        ] = predict

        out_content = [series_id, raw_spacing[0]]
        if raw_mask is not None:
            predict_mask = np.zeros(
                [num_class, raw_image_shape[0], raw_image_shape[1], raw_image_shape[2]],
                np.uint8,
            )
            for i in range(num_class):
                predict_mask[i] = out_mask == i + 1
            t_area_dice, t_surface_dice = compute_flare_metric(
                raw_mask, predict_mask, raw_spacing
            )
            total_area_dice = 0
            ...
            for i in range(num_class):
                ...
                self.logger.info(
                    "{} DSC: {}, NSC: {}".format(
                        self.cfg.DATA_LOADER.LABEL_NAME[i],
                        t_area_dice[i],
                        t_surface_dice[i],
                    )
                )
            out_content.extend(
                [total_area_dice / num_class, total_surface_dice / num_class]
            )
        else:
        ...
        if self.cfg.TESTING.IS_SAVE_MASK:
            ...
            save_ct_from_npy(
                out_mask, mask_path, spacing=raw_spacing, use_compression=True
            )
            self.logger.info("save fine mask complete!")

        out_content.extend(
            [
                float(time_usage["data_loader"]),
                float(time_usage["coarse_infer"]),
                float(time_usage["coarse_postprocess"]),
                float(time_usage["fine_infer"]),
            ]
        )
