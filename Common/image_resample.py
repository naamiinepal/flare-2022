...
class ItkResample(object):
    ...
    def resample_to_spacing(
        ...
        target_size = np.asarray(
            np.ceil(np.round(np.multiply(zoom_factor, source_size), decimals=5)),
            dtype=np.int16,
        )
        reference_image = self._sitk_new_blank_image(
            size=target_size,
        ...
        return self._sitk_resample_to_image(
            itk_image,
            reference_image,
    ...
    def resample_to_size(
        self, itk_image, target_size, interpolator=sitk.sitkLinear, default_value=0.0
    ):
        source_spacing = itk_image.GetSpacing()
        source_size = itk_image.GetSize()
        ...
        for i in range(3):
            target_spacing[i] = (
                source_size[i] * 1.0 * source_spacing[i] / target_size[i]
            )

        blank_image = self._sitk_new_blank_image(
            size=target_size,
            spacing=target_spacing,
            direction=itk_image.GetDirection(),
    ...
    def _sitk_new_blank_image(
        self, size, spacing, direction, origin, default_value=0.0
    ):
        itk_image = sitk.GetImageFromArray(
            np.ones(size, dtype=np.float32).T * default_value
        )
