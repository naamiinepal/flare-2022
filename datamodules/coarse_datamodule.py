...
class CoarseDataModule(pl.LightningDataModule):
    ...
    def get_weak_aug(
        ...
        return (
            Rand3DElasticd(
                ...
                padding_mode="zeros",
                rotate_range=(rot_angle, rot_angle, rot_angle),
    ...
    def get_strong_aug():
        return Compose(
            (
                ...
                RandScaleIntensity(factors=(0.7, 1.3), prob=0.95),
                RandAdjustContrast(gamma=(0.65, 1.5), prob=0.95),
                SimulateLowResolution(zoom_range=0.5, prob=0.95),
            )
        )
    ...
    def get_dataset(self, *dataset_args):
        if self.hparams.ds_cache_type == "mem":
            ...
            descrip = "hull" if self.hparams.use_hull else "nohull"
            return PersistentDataset(
                *dataset_args,
                cache_dir=(
                    f"{os.path.basename(self.hparams.supervised_dir)}_"
                    f"{descrip}_{__name__}_datacache"
                ),
                pickle_protocol=5,
            )
