...
def run_prepare_data(cfg):
    ...
    for data in data_prepare.data_info:
        try:
            ...
            traceback.print_exc()
            print(
                "Create coarse/fine image/mask throws exception %s, with series_id %s!"
                % (err, data.series_id)
            )
...
class MaskData(object):
    def __init__(
        ...
        image_path,
        mask_path,
        smooth_mask_path=None,
        coarse_image_path=None,
        coarse_mask_path=None,
        fine_image_path=None,
        fine_mask_path=None,
    ):
...
class DataPrepare(object):
    def __init__(self, cfg):
        ...
        if (
            self.out_coarse_size is not None and self.out_coarse_spacing is not None
        ) or (self.out_coarse_size is None and self.out_coarse_spacing is None):
            print(
                "One and just one can be set not none between",
                "out_coarse_size and out_coarse_spacing!",
            )
            return

        if self.out_coarse_size is not None:
            coarse_prefix = "{}_{}_{}".format(
                self.out_coarse_size[0],
                self.out_coarse_size[1],
                self.out_coarse_size[2],
            )
        else:
            coarse_prefix = "{}_{}_{}".format(
                self.out_coarse_spacing[0],
                self.out_coarse_spacing[1],
                self.out_coarse_spacing[2],
            )

        self.coarse_image_save_dir = os.path.join(
            self.out_dir, "coarse_image", coarse_prefix
        )
        if not os.path.exists(self.coarse_image_save_dir):
            os.makedirs(self.coarse_image_save_dir)

        self.coarse_mask_save_dir = os.path.join(
            self.out_dir, "coarse_mask", coarse_prefix
        )
        ...
        if (self.out_fine_size is not None and self.out_fine_spacing is not None) or (
            ...
            print(
                "One and just one can be set not none",
                "between out_fine_size and out_fine_spacing!",
            )
            return

        if self.out_fine_size is not None:
            fine_prefix = "{}_{}_{}".format(
                self.out_fine_size[0], self.out_fine_size[1], self.out_fine_size[2]
            )
        else:
            fine_prefix = "{}_{}_{}".format(
                self.out_fine_spacing[0],
    ...
    def process(self, data):
        ...
        if npy_image.shape != npy_mask.shape:
            print(
                "Shape of image/mask are not equal in series_id: {}".format(
                    data.series_id
                )
            )
            return
        ...
        #             area_least = int(
        #                    2000 / mask_spacing[0] / mask_spacing[1] / mask_spacing[2]
        #                   )
        #             smooth_mask(
        #               t_mask, t_smooth_mask, out_num_label=i,
        #               area_least=area_least, is_binary_close=False)
        #         npy_mask = t_smooth_mask.copy()
        #         np.save(data.smooth_mask_path, t_smooth_mask)

        # Process and save coarse image and mask
        if self.out_coarse_size is not None:
            coarse_image, _ = ScipyResample.resample_to_size(
                npy_image, self.out_coarse_size
            )
            coarse_mask, _ = ScipyResample.resample_mask_to_size(
                npy_mask, self.out_coarse_size, num_label=num_label
            )
        else:
            coarse_image, _ = ScipyResample.resample_to_spacing(
                npy_image, image_spacing, self.out_coarse_spacing
            )
            coarse_mask, _ = ScipyResample.resample_mask_to_spacing(
                npy_mask, mask_spacing, self.out_coarse_spacing, num_label=num_label
            )
        ...
        t_crop_image, t_crop_mask = crop_image_according_to_mask(
            npy_image, npy_mask, margin
        )

        if self.out_fine_size is not None:
            fine_image, _ = ScipyResample.resample_to_size(
                t_crop_image, self.out_fine_size
            )
            fine_mask, _ = ScipyResample.resample_mask_to_size(
                t_crop_mask, self.out_fine_size, num_label=num_label
            )
        else:
            fine_image, _ = ScipyResample.resample_to_spacing(
                t_crop_image, image_spacing, self.out_fine_spacing
            )
            fine_mask, _ = ScipyResample.resample_mask_to_spacing(
                t_crop_mask, mask_spacing, self.out_fine_spacing, num_label=num_label
            )
    ...
    def _read_db(self):
        ...
        for key, value in txn.cursor():
            ...
            tmp_data = MaskData(key, label_info["image_path"], label_info["mask_path"])
    ...
    def _update_db(self, data):
        ...
        txn.put(
            str(data.series_id).encode(),
            value=json.dumps(data_dict, cls=MyEncoder).encode(),
        )
    ...
    def _create_db_file(self, phase="train"):
        ...
        series_ids = (
            read_txt(self.cfg.DATA_PREPARE.TRAIN_SERIES_IDS_TXT)
            if phase == "train"
            else read_txt(self.cfg.DATA_PREPARE.TEST_SERIES_IDS_TXT)
        )
        print(series_ids)
        ...
        for series_id in series_ids:
            if phase == "train":
                image_path = os.path.join(
                    self.cfg.DATA_PREPARE.TRAIN_IMAGE_DIR, series_id + "_0000.nii.gz"
                )
                mask_path = os.path.join(
                    self.cfg.DATA_PREPARE.TRAIN_MASK_DIR, series_id + ".nii.gz"
                )
                if os.path.exists(image_path) and os.path.exists(mask_path):
                    data_dict = {"image_path": image_path, "mask_path": mask_path}
                    DataBaseUtils.update_record_in_db(
                        db_file_path, series_id, data_dict
                    )
                else:
                    print("%s has invalid image/mask." % series_id)
            else:
                image_path = os.path.join(
                    self.cfg.DATA_PREPARE.TEST_IMAGE_DIR, series_id + "_0000.nii.gz"
                )
                mask_path = (
                    os.path.join(
                        self.cfg.DATA_PREPARE.TEST_MASK_DIR, series_id + ".nii.gz"
                    )
                    if self.cfg.DATA_PREPARE.TEST_MASK_DIR is not None
                    else None
                )
    ...
    def _split_train_val(self):
        ...
        if default_train_db is not None and default_val_db is not None:
            ...
            series_ids_train, series_ids_val = train_test_split(
                series_ids, test_size=self.cfg.DATA_PREPARE.VAL_RATIO, random_state=0
            )

        print(
            "Num of train series is: %d, num of val series is: %d."
            % (len(series_ids_train), len(series_ids_val))
        )
        print(series_ids_train)
        # create train db
    ...
    def _split_5fold_train_val(self):
        ...
        new_train_series_uid = [
            default_train_series_uid[: int(num_train * 0.25)],
...
def generate_heatmap(
    npy_mask, candidates_num, sigma=5, stride=4, is_single_channel=False
):
    ...
    for i in range(channel):
        centroid = extract_candidate_centroid(
            npy_mask[i].copy(), 100, candidates_num[i]
        )
        if len(centroid) != candidates_num[i]:
            centroid.extend(
                [[0, 0, 0] for _ in range(candidates_num[i] - len(centroid))]
            )
    ...
    if is_single_channel:
        target = np.zeros(
            (1, heatmap_size[0], heatmap_size[1], heatmap_size[2]), dtype=np.float32
        )
    else:
        target = np.zeros(
            (num_joints, heatmap_size[0], heatmap_size[1], heatmap_size[2]),
            dtype=np.float32,
        )
    ...
    for joint_id in range(num_joints):
        ...
        for xyz_coord in joint_centroid:
            ...
            if is_single_channel:
                target[
                    0, img_z[0] : img_z[1], img_y[0] : img_y[1], img_x[0] : img_x[1]
